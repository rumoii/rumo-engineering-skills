...
[ -n "$REPO_PATH" ] || { [ -d "$SCRIPT_DIR/skills" ] && REPO_PATH="$SCRIPT_DIR" || REPO_PATH="$HOME/.rumo-skills"; }
...
if [ "$CLAUDE_HOME_EXPLICIT" -eq 1 ] || command -v claude >/dev/null 2>&1 || [ -d "$CLAUDE_HOME_DIR" ]; then
  sync_links "$CLAUDE_HOME_DIR/skills"
else
  echo 'Claude Code was not detected; skipping.'
fi
if [ "$AGENTS_HOME_EXPLICIT" -eq 1 ] || command -v grok >/dev/null 2>&1 || [ -d "$HOME/.grok" ]; then
  sync_links "$AGENTS_HOME_DIR/skills"
else
  echo 'A shared agent client was not detected; skipping.'
fi
